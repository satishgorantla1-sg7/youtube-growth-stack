...
const idea = { id: "71000000-5000-4000-8000-000000000001", title: "Evidence-led growth", premise: "A grounded premise", evidenceCount: 1 };
...
describe("PackageWorkbench", () => {
  ...
  it("shows approved-idea generation and immutable evidence provenance", () => {
    render(<PackageWorkbench approvedIdeas={[idea]} packages={[packageVersion]} canGenerate canDecide/>);
  ...
  it("keeps export visibly disabled and non-executable", () => {
    render(<PackageWorkbench approvedIdeas={[idea]} packages={[]} canGenerate canDecide/>);
  ...
  it("shows approval decisions only to owners and admins", () => {
    const { rerender } = render(<PackageWorkbench approvedIdeas={[idea]} packages={[packageVersion]} canGenerate canDecide={false}/>);
    expect(screen.queryByRole("button", { name: /Approve version/ })).not.toBeInTheDocument();
    rerender(<PackageWorkbench approvedIdeas={[idea]} packages={[packageVersion]} canGenerate canDecide/>);
